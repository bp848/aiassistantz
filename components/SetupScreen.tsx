...
const SetupScreen: React.FC<SetupScreenProps> = ({ onSelectMode, onOpenFileWarehouse, onOpenHistory, onLogout }) => {
  ...
  const [tokenDiagnostics, setTokenDiagnostics] = React.useState<{
    sessionProviderToken: boolean;
    sessionProviderRefreshToken: boolean;
    localAccessToken: boolean;
    localRefreshToken: boolean;
  } | null>(null);
  ...
  React.useEffect(() => {
    const loadDiagnostics = async () => {
      ...
      setTokenDiagnostics({
        sessionProviderToken: !!session?.provider_token,
        sessionProviderRefreshToken: !!session?.provider_refresh_token,
        localAccessToken: !!localStorage.getItem('google_access_token'),
        localRefreshToken: !!localStorage.getItem('google_refresh_token')
      });
    };
    loadDiagnostics();
  }, []);
  ...
  return (
    <div className="min-h-[100dvh] bg-[#0F172A] flex flex-col items-center justify-center p-4 text-gray-100 relative overflow-hidden">
      ...
      <div className="absolute bottom-8 left-0 right-0 flex flex-col items-center gap-3 w-full px-6">
        ...
        {tokenDiagnostics && (
          <div className="bg-black/60 backdrop-blur-xl border border-white/10 p-4 rounded-2xl shadow-2xl max-w-md w-full text-[11px] text-gray-300">
            <div className="mb-2 text-[10px] uppercase tracking-wider text-gray-400">Google Token Diagnostics</div>
            <div className="grid grid-cols-2 gap-x-4 gap-y-1">
              <div>session.provider_token</div>
              <div className={tokenDiagnostics.sessionProviderToken ? 'text-green-400' : 'text-red-400'}>
                {tokenDiagnostics.sessionProviderToken ? 'present' : 'missing'}
              </div>
              <div>session.provider_refresh_token</div>
              <div className={tokenDiagnostics.sessionProviderRefreshToken ? 'text-green-400' : 'text-red-400'}>
                {tokenDiagnostics.sessionProviderRefreshToken ? 'present' : 'missing'}
              </div>
              <div>localStorage.google_access_token</div>
              <div className={tokenDiagnostics.localAccessToken ? 'text-green-400' : 'text-red-400'}>
                {tokenDiagnostics.localAccessToken ? 'present' : 'missing'}
              </div>
              <div>localStorage.google_refresh_token</div>
              <div className={tokenDiagnostics.localRefreshToken ? 'text-green-400' : 'text-red-400'}>
                {tokenDiagnostics.localRefreshToken ? 'present' : 'missing'}
              </div>
            </div>
          </div>
        )}
